...
{
    int i, j;
    #pragma omp target is_device_ptr(src)
    #pragma omp teams distribute parallel for default(shared) //collapse(2)
    for(i = 0 ; i < col ; i++) //i->col
    {
        for(j = 0 ; j < row ; j++) //j->row
...
void updateBlockVector_GPU(double *activeBlockVectorR, int *activeMask, double *blockVectorR, int M, int blocksize, int currentBlockSize)
{
    ...
    int i, j, k = 0;
    #pragma omp target
    #pragma omp teams distribute parallel for private(k, j) default(shared)
    for(i = 0 ; i < M ; i++)
    {
        k = 0 ;
        for(j = 0 ; j < blocksize ; j++)
...
{
    int i, j;
    #pragma acc parallel loop deviceptr(src1, src2, dst) collapse(2)
    for(i = 0; i < row ; i++)
    {
        //#pragma acc loop
        for(j = 0 ; j < col ; j++)
...
void custom_dlacpy_OpenACC(double *src, double *dst, int m, int n)
{
    //src[m*n] and dst[m*n]
    int i, j;
    #pragma acc parallel loop deviceptr(src, dst) collapse(2)
    for(i = 0 ; i < m ; i++) //each row
    {
        // #pragma acc loop
        for(j = 0 ; j < n ; j++) //each column
