...
int main(int argc, char *argv[])
{
    ...
    // acsrfile.open ("HV15R_acsr.txt");
    // jafile.open ("HV15R_ja.txt");
    // iafile.open ("HV15R_ia.txt");
   ...
   //#pragma omp parallel for private(j) default(shared)
    ...
    double *trans_PAP = (double *) malloc(currentBlockSize * currentBlockSize * sizeof(double));
    ...
    std::memset(zeros_B_CB, 0.0, currentBlockSize * blocksize * sizeof(double));
    std::memset(zeros_CB_B, 0.0, currentBlockSize * blocksize * sizeof(double));
    ...
    for(iterationNumber = 1 ; iterationNumber <= maxIterations ; iterationNumber++)
    {
        ...
        for(int cond_try = 1 ; cond_try <=2 ; cond_try++)
        {
            if(restart == 0) //---- if 24 ----
            {
                if(restart == 0)
                {
                    ...
                    // printf("**---------------- Printing identity_PAP 1 ----------------**\n");
                    // print_mat(identity_PAP, currentBlockSize, currentBlockSize);
                    // printf("\n");
                    
                    
                    transpose(gramPAP, trans_PAP, currentBlockSize, currentBlockSize);
                    #pragma acc parallel loop gang vector collapse(2)
                    for(i = 0 ; i < currentBlockSize ; i++)
                    {
                        for(j = 0 ; j < currentBlockSize ; j++)
                        {
                            gramPAP[i * currentBlockSize + j] = 0.5 * (gramPAP[i * currentBlockSize + j] + trans_PAP[i * currentBlockSize + j]);
                        }
                    }
        ...
        //     gafile.open ("ga_v2_itr_2.txt");
        //     gbfile.open ("gb_v2_itr_2.txt");
        ...
        {   
            // #pragma acc parallel for default(shared)
            for(i = 0 ; i < gramASize ; i++)
            {
